...
function processProposals(data, tableElements, countElements) {
  ...
  const statusCategories = {
    ...
    refinAguardaDigitacao: ["2", "1", "16"],
    refinAguardaOperacional: ["9", "15"],
    aguardaPort: ["17"],
    andamentoRefin: ["19", "5"],
    ...
    if(element.status == '13'){
        ...
        <tr id="row-proposal-${element.id}" data-bound-proposal="${element.bound_proposal}">
            ...
            <td>
                ...
                <button class="btn btn-secondary btn-sm" title="Status" disabled>
...
async function changeStatusDigitacao() {
  ...
  if(adePort != '' && adePort === adeRefin){
    addNewToast("danger", "ADE de Portabilidade e Refinanciamento devem ser diferentes")
  } else {
  if(status == 4){
    newDataPort = {
      ade: adePort,
