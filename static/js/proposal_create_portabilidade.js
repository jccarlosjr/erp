...
function setTablePort(selectElement, data) {
...
function getChange() {
  ...
  let ballance = parseFloat(document.getElementById("ballance").value) || 0;

  let exchange = document.getElementById("exchange");

  if (coefficient > 0) {
    let new_change = installment / coefficient - ballance;
    exchange.value = new_change.toFixed(2);
  } else {
    exchange.value = "0.00";
  }
}
...
function getTotalAmount() {
  ...
  if (coefficient > 0) {
    let new_total_amount = installment / coefficient;
...
async function getTableByOperationAndBank(bank_id){
  const token = localStorage.getItem("access_token");
  const operationRefin = 4;
  const operationPort = 3;
  const bankId = parseInt(bank_id)

  try {
    const response = await fetch(`/api/v1/table/operation/${operationPort}/bank/${bankId}/`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    const data = await response.json();
    const tableSelect = document.getElementById("table_port");
    setTablePort(tableSelect, data);
  ...
  try {
    const response = await fetch(`/api/v1/table/operation/${operationRefin}/bank/${bankId}/`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    const data = await response.json();
    const tableSelect = document.getElementById("table_refin");
    setTableRefin(tableSelect, data);
...
async function boundProposal(id, id_proposal_to_bound){
  const csrftoken = getCSRFToken();
  const token = localStorage.getItem("access_token");

  data = {
    bound_proposal: id_proposal_to_bound
  }
...
async function postNewProposal(event) {
  ...
  let term_paids = parseInt(document.getElementById("term_paids").value)
  let term_original = parseInt(document.getElementById("term_original").value)
  ...
  const dataPort = {
    ...
    table: document.getElementById("table_port").value,
    user: user_id,
    installment: parseFloat(document.getElementById("installment").value),
    ballance: parseFloat(document.getElementById("ballance").value),
    total_amount: parseFloat(document.getElementById("total_amount").value),
    exchange: parseFloat(0),
    term: parseInt(termPort),
    term_paids: parseInt(document.getElementById("term_paids").value),
  ...
  const dataRefin = {
    ...
    table: document.getElementById("table_refin").value,
    user: user_id,
    installment: parseFloat(document.getElementById("installment").value),
    ballance: parseFloat(document.getElementById("ballance").value),
    total_amount: parseFloat(document.getElementById("total_amount").value),
  ...
  let idPort = await postProposal(event, dataPort);
  let idRefin = await postProposal(event, dataRefin);

  boundProposal(idPort, idRefin);
  boundProposal(idRefin, idPort);
...
function dataCheck() {
  const fields = [
    ...
    { id: "ballance", name: "Saldo Devedor" },
    { id: "total_amount", name: "Valor Financiado" },
    { id: "exchange", name: "Troco" },
    { id: "term", name: "Prazo do Refinanciamento" },
    { id: "term_paids", name: "Parcelas Pagas" },
    { id: "term_original", name: "Prazo Original" },
    { id: "contract", name: "Contrato" },
    { id: "original_bank", name: "Banco Originador" },
  ];

  let table_port = document.getElementById("table_port").value;
  if(table_port == "0"){
    addNewToast("danger", "Selecione uma tabela de portabilidade")
...
let table_refin = document.getElementById("table_refin");
let installment = document.getElementById("installment");
let ballance = document.getElementById("ballance");

table_refin.addEventListener("click", () => {
  getTotalAmount();
  getChange();
});
ballance.addEventListener("blur", () => {
  getTotalAmount();
  getChange();
});
installment.addEventListener("blur", () => {
  getTotalAmount();
  getChange();
});
