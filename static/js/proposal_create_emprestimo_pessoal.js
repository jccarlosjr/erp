...
function setTableMargem(selectElement, data) {
...
function getChange() {
  ...
  if (coefficient > 0) {
    let new_change = installment / coefficient;
...
async function getTableByOperationAndBank(bank_id){
  const token = localStorage.getItem("access_token");
  const operationMargem = 8;
  const bankId = parseInt(bank_id)

  try {
    const response = await fetch(`/api/v1/table/operation/${operationMargem}/bank/${bankId}/`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    const data = await response.json();
    const tableSelect = document.getElementById("table_margem");
    setTableMargem(tableSelect, data);
...
async function postNewProposal(event) {
  ...
  const data = {
    ...
    table: document.getElementById("table_margem").value,
    user: user_id,
  ...
  let idMargem = await postProposal(event, data);
...
function dataCheck() {
  ...
  let table_margem = document.getElementById("table_margem").value;
  if(table_margem == "0"){
...
let table = document.getElementById("table_margem");
